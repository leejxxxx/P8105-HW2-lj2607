...
The dataset has `r ncol(nyc_transit_clean)` columns and `r nrow(nyc_transit_clean)` rows. The dimenison of the resulting data set is : `r dim(nyc_transit_clean)`. The dataset contain variables : `r names(nyc_transit_clean)`. The data are not tidy because the route number is spread across 11 columns and 
...
trash_wheel_data is composed of three datasets:mr_trash_data, prof_trash_data, gwynnda_trash_data.
There are `r nrow(trash_wheel_data)` rows and `r ncol(trash_wheel_data)` in dataset
and key variable include `r name(trash_wheel_data)`. The number of observation in datset is `r nrow(trash_wheel_data)`. The total weight of trash collected by professor trash wheel is 246.74 tons. The total number of cigarette butts collected by Gwynnda in June of 2022 is 18120.
